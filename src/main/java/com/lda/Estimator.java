...
public class Estimator {
	...
	public void estimate(){
		...
		for (trnModel.liter = lastIter + 1; trnModel.liter < trnModel.niters + lastIter; trnModel.liter++){
			...
			// for all z_i
			for (int m = 0; m < trnModel.M; m++){// for each document d				
				for (int n = 0; n < trnModel.data.docs[m].length; n++){ // for each word in document d
					// z_i = z[m][n]
	...
	public int sampling(int m, int n){
		...
		int topic = trnModel.z[m].get(n);//拿到文档m中第n个词的topic
		int w = trnModel.data.docs[m].words[n];//w是这个词的序号
